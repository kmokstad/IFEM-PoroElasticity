...
ThermoPoroElasticity::ThermoPoroElasticity(unsigned short int n, int order, bool stab) :
  PoroElasticity(n), gacc(9.81), SUPG(stab)
{
                                     ...
                                     const TimeDomain& time, const Vec3& X) const
{
  ...
  PoroMaterial* mat = static_cast<PoroMaterial*>(this->getMaterial());
  ...
  SymmTensor Kperm(nsd);
  for (size_t i = 1; i <= nsd; i++)
    Kperm(i,i) = hydCond(i);

  // Integration of Cuu
  Matrix CB;
  CB.multiply(Cmat,Bmat).multiply(-1.0*fe.detJxW); // CB = dSdE*B*|J|*w
  elMat.A[uu].multiply(Bmat,CB,true,false,true); // K_uu += B^T * CB

  // Integration of Cup
  if (!this->evalCouplingMatrix(elMat.A[up], Bmat, fe.basis(2),
                                scl1*alpha*fe.detJxW))
    return false;

  double T = elMat.vec[Tc].dot(fe.basis(2));
  double rhoc = mat->getHeatCapacity(T);
  double lambda = mat->getThermalConductivity(T);
  size_t nbf = fe.basis(2).size();

  // Integration of CuT
  Matrix BC;
  BC.multiply(Bmat,Cmat,true,false);
  double alpha_s = mat->getSolidThermalExpansion(T);
  if (!this->evalCouplingMatrix(elMat.A[uT], BC, fe.basis(2),
                                scl2*alpha_s/3.0*fe.detJxW,false))
    return false;

  // Integration of Cpp
  Matrix Cpp(nbf,nbf);
  if (!this->evalCompressibilityMatrix(Cpp, fe.basis(2),
                                       scl1*scl1*Minv*fe.detJxW))
    return false;

  // Integration of Kpp
  Matrix Kpp(nbf,nbf);
  this->evalPermeabilityMatrix(Kpp, fe.grad(2),
                               Kperm, scl1*scl1*fe.detJxW/(rhof*gacc));
  ...
  // Integration of CTT
  Matrix CTT;
  CTT.outer_product(fe.basis(2),fe.basis(2),false,scl2*scl2*rhoc);
  ...
  Vec3 vel;
  for (int k = 1; k <= nsd; k++)
    vel[k-1] = -gradP(k)*hydCond[k-1]/(rhof*gacc);

  Matrix KTT;
  KTT.multiply(fe.grad(2),fe.grad(2),false,true,false,
               scl2*scl2*lambda*fe.detJxW);

  double advc = scl2*scl2*rhof*mat->getFluidHeatCapacity(T)*fe.detJxW;
  for (size_t i = 1; i <= nbf; i++)
    for (size_t j = 1; j <= nbf; j++)
      for (size_t k = 1; k <= fe.grad(2).cols(); k++)
        KTT(i,j) += advc*vel[k-1]*fe.grad(2)(j,k)*fe.basis(2)(i);
  ...
  if (SUPG)
  {
    Matrix KTTs(nbf,nbf), CTTs(nbf,nbf);
    //TODO Include this element size calculation in FiniteElement
    //double h = fe.getElementSize(nsd);
    //double h = StabilizationUtils::getElementSize(fe.XC,nsd);
    double h = fe.h; //tmp
    double alpha_e = h*fabs(nsd*vel[0])/(2*lambda);
    double taue = h/(2*fabs(nsd*vel[0])) * (1/tanh(alpha_e) - 1/alpha_e);
    std::cout << "h = " << fe.h << ", alpha_e = " << alpha_e << ", taue = " << taue << std::endl;
    for (size_t i = 1; i <= nbf; i++) {
      for (size_t j = 1; j <= nbf; j++) {
                                           ...
                                           const TimeDomain&, size_t)
{
  ...
  for (i = 1; i <= ru; i++)
  {
    ...
    for (j = 1, kj = ru+1; j <= rp; j++, kj += 2)
    {
      elMat.A[Kprev](i,kj) = elMat.A[up](i,j);
      elMat.A[Know](i,kj)  = elMat.A[up](i,j);
      elMat.A[Kprev](kj,i) = elMat.A[up](i,j);
      elMat.A[Know](kj,i)  = elMat.A[up](i,j);
      elMat.A[Kprev](i,kj+1) = elMat.A[uT](i,j);
      elMat.A[Know](i,kj+1)  = elMat.A[uT](i,j);
      //elMat.A[Kprev](kj+1,i) = elMat.A[uT](i,j);
      //elMat.A[Know](kj+1,i)  = elMat.A[uT](i,j);
    }
  }

  for (i = 1, ki = ru+1; i <= rp; i++, ki += 2)
    for (j = 1, kj = ru+1; j <= rp; j++, kj += 2) {
      elMat.A[Know](ki  ,kj)   = elMat.A[pp](i,j);
      elMat.A[Know](ki+1,kj+1) = elMat.A[TT](i,j);
    }

  Vector PoTo(2*elMat.vec[Po].size());
  Vector PcTc(2*elMat.vec[Pc].size());
  ...
  Vector prevSol(elMat.vec[Uo]);
  Vector currSol(elMat.vec[Uc]);
...
{
  static const char* s[] = { "u_x", "u_y", "p^w", "T" };
  if (i == 11)
    return "first_basis";
  else if (i == 12)
    return "second_basis";
...
std::string ThermoPoroElasticity::getField2Name (size_t i, const char* prefix) const
{
